...
**Status revised 2026-04-27 — P0-5 ships tool-only.** The original plan was to capture 5 × 30 s WAV recordings + a 5-minute idle Chrome DevTools Performance JSON, then have every audio-touching stage (P3-5, P3-6, P5-1) compare against those artifacts via `ffmpeg loudnorm` + spectrogram diff + RMS comparison. By user decision (logged in the P0-5 devlog), the recording capture step was skipped — the recorder file (`scripts/record-audio-baseline.html`) is committed for opt-in use, but no clips were captured at P0-5.

**What this changes** for every audio-touching stage:

- **P3-5 / P3-6 DoD**: drop the "ΔLUFS ≤ 0.5 LU + spectrogram + RMS Δ ≤ 1%" hard gate. Replace with: (a) vitest coverage on the extracted module (asserting routing topology and parameter values via the P0-1 audio-context mock), (b) a manual A/B listen on `npm run dev` for ≥ 30 s with audio settled, listening for clicks / pops / gain steps / pitch shifts, and (c) a "what nodes connect to what" review in the stage's devlog, naming AudioNode types and connection order.
- **P5-1 DoD**: capture the idle CPU baseline _at the time P5-1 lands_ (against the M4 codebase as it stands at that point) rather than against a P0-5 baseline. The comparison still proves the idle-detection patch reduces CPU; it does not bound the absolute number against an M3 reference.
- **P5-4 DoD**: same — the closing audit relies on the local A/B listening protocol, not on `ffmpeg`-based numeric audio comparison.

The recorder file (`scripts/record-audio-baseline.html`) remains in-tree as opt-in infrastructure: any operator who wants the full numeric comparison can run it from the same origin as the dev server, archive WebM/Opus clips under `~/Documents/M4-baseline/`, and append numbers to `docs/plans/M4/baseline/baseline-notes.md`. Doing so is reversible and does not require a stage rerun.
...
| **P0-5** | **Tool-only delivery (revised 2026-04-27).** Build `scripts/record-audio-baseline.html` (one button per city, MediaRecorder + auto-download) — opt-in for any operator who later wants full audio capture. Run `npm run benchmark` against the running dev server and archive the four-scenario p50/p95/p99 numbers in `docs/plans/M4/baseline/baseline-notes.md`. Record post-P0-3/P0-4 LOC + test counts. Audio recordings + idle CPU JSON deliberately skipped (operator opted out; replacement protocol in §2.E). | New `scripts/record-audio-baseline.html`, new `docs/plans/M4/baseline/baseline-notes.md` (text-only into git; no external WAV / JSON archive at this stage) | 2h | Recorder file ready in-tree; `baseline-notes.md` lists server `viewport-processor` p95 + LOC + test counts at HEAD; the audio + idle-CPU rows of §11 quantitative targets are reinterpreted per §2.E revision. |

**Phase Gate (local — no prod merge).** `npm test && npm run test:frontend && npm run lint && npm run smoke:wire-format` all green; `audio-engine.js` shrinks 1186 → 1124 lines (P0-3 only — further drops scheduled for P3); baseline-notes.md present with server benchmark numbers (audio recordings deferred per §2.E revision); **5 devlog entries** committed to `feat/M4`; the local Chinese plan carries a "superseded" notice and this proposal takes over. P0 work continues toward P1 on the same `feat/M4` branch with no prod soak.
...
| Stage | Task | Critical files | Hours | DoD |
|---|---|---|---|---|
| **P5-1** | Add idle detection to `audio/raf-loop.js`: when every EMA has converged (Δ < 0.001) and velocity = 0, skip the frame; preserve a wake mechanism. | `frontend/audio/raf-loop.js` | 2h | Capture an at-time idle CPU baseline (5-min DevTools Performance recording on `npm run dev` _before_ the patch), apply the patch, capture a second 5-min recording, and verify main-thread CPU drops (target: drop ≥ 30%, OR absolute value ≤ 3.5%, whichever first). Convergence speed during interaction unchanged. §2.E manual A/B listen confirms no audible regression (revised — no P0-5 5-segment reference). |
...
| Cold-start FCP | (no captured baseline; tool-only P0-5) | informal local check, no regression report | Manual DevTools recording at P5-4 if desired |
| Audio idle main-thread CPU median | (no P0-5 baseline) | P5-1 captures a fresh M4-then-current baseline at the time the patch lands and verifies the patch drops it; absolute target dropped | DevTools 5-min idle Performance recording at P5-1 |
| Audio LUFS / spectrogram / waveform RMS | (no captured baseline) | manual A/B listen on `npm run dev` (see §2.E) | unit-test routing topology + listener QA + code review |
...
|---|---|---|---|
| **P5-4 big-bang merge surfaces multiple latent regressions at once (single-branch cadence trade-off)** | **Critical** | Every prior stage's local Phase Gate stays strict; `feat/M4` keeps `npm test && npm run test:frontend && npm run lint && npm run smoke:wire-format` green at every commit; §2.E manual A/B listen run at P5-4 against `feat/M4` HEAD (no captured P0-5 reference per §2.E revision); preview-URL stress tested before marking PR ready | The draft PR can sit in "ready for review" indefinitely; if smoke uncovers a bug, fix on `feat/M4` and re-validate before merge. Worst case: tag the fail point, revert the merge, and bisect on `feat/M4` |
| P3-4/P3-5 swap-timer / bus split causes crossfade-timing glitches (clicks / pops) | **Critical** | P0 test scaffold + 30-min local stress + §2.E manual A/B listen + feature flag kept on `feat/M4` until P5-4 cleanup (revised — replaces the spectrogram comparison) | Toggle the flag for instant fallback on `feat/M4`; worst case revert the P3 commits before merge to `main` |
...
| P2-5 lifecycle rewrite introduces a startup-order regression | Medium | Local FCP feels unchanged on `npm run dev` (subjective; no P0-5 FCP baseline captured); P2-3's dry-run comment vetoes designs that don't match `main.js` reality | Revert P2-5 commits on `feat/M4` |
| Audio-engine ends up half-extracted, half-tested mid-phase | Medium | §2.E manual A/B listen on every P3 stage before commit (revised — replaces the P0-5 audio-reference diff) | Stage-level revert on `feat/M4` |
...
| P3 | P2 commits passing locally + audio/ subsystem coverage ≥ 70% (P3 raises this stage by stage) + §2.E manual A/B listening protocol locked in (revised — replaces "P0-5 baseline captured") | Nothing in parallel (audio core ownership). |
