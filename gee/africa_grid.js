// ========== Africa Grid (MVP: landcover + nightlight + population) ==========
// Paste into GEE Code Editor and run. Data vintage: 2020-2021.

// South boundary extended to -41 to cover Tristan da Cunha (~37°S) and Gough Island (~40°S)
var region = ee.Geometry.Rectangle([-18, -41, 52, 38]);
var gridSize = 0.5;
// Tuning knobs
...
var lonList = ee.List.sequence(-18, 52 - gridSize, gridSize);
var latList = ee.List.sequence(-41, 38 - gridSize, gridSize);
...
// var testRegion = ee.Geometry.Rectangle([0, 0, 10, 10]);
...
// print('Africa grids:', results.size());  // Skip .size() to avoid memory limit on large regions
Map.centerObject(region, 3);
Export.table.toDrive({
  collection: results, description: 'africa_grid', folder: 'geo_sonification',
  fileNamePrefix: 'africa_grid', fileFormat: 'CSV',
  selectors: [
    'grid_id', 'lon', 'lat',
    'landcover_class',
