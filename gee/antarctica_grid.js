// ========== Antarctica Grid (MVP: landcover + nightlight + population) ==========
// Paste into GEE Code Editor and run. Data vintage: 2020-2021.
// Note: Antarctica has near-zero population and nightlight; most cells will be ice/bare land.

// Start at -85 instead of -90 to avoid degenerate geometry at the South Pole singularity.
// Antarctic coastline is at ~60-70S; -85 covers all land with margin.
var region = ee.Geometry.Rectangle([-180, -85, 180, -60]);
var gridSize = 0.5;
// Tuning knobs
...
var lonList = ee.List.sequence(-180, 180 - gridSize, gridSize);
var latList = ee.List.sequence(-85, -60 - gridSize, gridSize);
...
// var testRegion = ee.Geometry.Rectangle([160, -78, 170, -68]);
...
// print('Antarctica grids:', results.size());  // Skip .size() to avoid memory limit on large regions
Map.centerObject(region, 3);
Export.table.toDrive({
  collection: results, description: 'antarctica_grid', folder: 'geo_sonification',
  fileNamePrefix: 'antarctica_grid', fileFormat: 'CSV',
  selectors: [
    'grid_id', 'lon', 'lat',
    'landcover_class',
