// ========== Europe Grid (MVP: landcover + nightlight + population) ==========
// Paste into GEE Code Editor and run. Data vintage: 2020-2021.

// Extended bounds to include Svalbard, Jan Mayen, and Azores western group (Flores/Corvo at ~31°W)
var region = ee.Geometry.Rectangle([-32, 35, 40, 82]);
var gridSize = 0.5;
// Tuning knobs
...
var lonList = ee.List.sequence(-32, 40 - gridSize, gridSize);
var latList = ee.List.sequence(35, 82 - gridSize, gridSize);
...
// var testRegion = ee.Geometry.Rectangle([5, 45, 15, 55]);
...
// print('Europe grids:', results.size());  // Skip .size() to avoid memory limit on large regions
Map.centerObject(region, 3);
Export.table.toDrive({
  collection: results, description: 'europe_grid', folder: 'geo_sonification',
  fileNamePrefix: 'europe_grid', fileFormat: 'CSV',
  selectors: [
    'grid_id', 'lon', 'lat',
    'landcover_class',
