// ========== North America Grid (MVP: landcover + nightlight + population) ==========
// Paste into GEE Code Editor and run. Data vintage: 2020-2021.

// Extended bounds to include Greenland and Arctic regions
var region = ee.Geometry.Rectangle([-170, 7, -10, 84]);
var gridSize = 0.5;
// Tuning knobs
...
var lonList = ee.List.sequence(-170, -10 - gridSize, gridSize);
var latList = ee.List.sequence(7, 84 - gridSize, gridSize);
...
// var testRegion = ee.Geometry.Rectangle([-110, 35, -100, 45]);
...
// print('North America grids:', results.size());  // Skip .size() to avoid memory limit on large regions
Map.centerObject(region, 3);
Export.table.toDrive({
  collection: results, description: 'north_america_grid', folder: 'geo_sonification',
  fileNamePrefix: 'north_america_grid', fileFormat: 'CSV',
  selectors: [
    'grid_id', 'lon', 'lat',
    'landcover_class',
