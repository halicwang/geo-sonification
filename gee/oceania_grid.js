// ========== Oceania Grid (MVP: landcover + nightlight + population) ==========
// Paste into GEE Code Editor and run. Data vintage: 2020-2021.

// Full longitude span to capture date-line-crossing Pacific islands (Samoa, Tonga, Cook Islands).
// Overlaps with S.America/Africa are handled by server-side deduplication.
// Most ocean cells are filtered out early by MIN_LAND_FRACTION.
//
// === PERFORMANCE: Due to ~72k cell grid spanning full 360° longitude, run this script
// 4 times with different longitude bands to avoid GEE timeouts:
//   Run 1: LON_START=-180, LON_END=-90   (export as oceania_grid_q1)
//   Run 2: LON_START=-90,  LON_END=0     (export as oceania_grid_q2)
//   Run 3: LON_START=0,    LON_END=90    (export as oceania_grid_q3)
//   Run 4: LON_START=90,   LON_END=180   (export as oceania_grid_q4)
// The server deduplicates by lon_lat, so overlapping cells are handled.
// After all 4 exports, concatenate into a single oceania_grid.csv if desired.
var LON_START = -180;  // <--- CHANGE THIS PER RUN (see table above)
var LON_END = -90;     // <--- CHANGE THIS PER RUN (see table above)

var region = ee.Geometry.Rectangle([LON_START, -50, LON_END, 0]);
var gridSize = 0.5;
// Tuning knobs
...
var lonList = ee.List.sequence(LON_START, LON_END - gridSize, gridSize);
var latList = ee.List.sequence(-50, 0 - gridSize, gridSize);

var grid = ee.FeatureCollection(
  lonList.map(function(lon) {
    return latList.map(function(lat) {
      var geom = ee.Geometry.Rectangle([lon, lat, ee.Number(lon).add(gridSize), ee.Number(lat).add(gridSize)]);
      return ee.Feature(geom, {
        lon: lon, lat: lat,
        grid_id: ee.String(ee.Number(lon).format('%.1f')).cat('_').cat(ee.Number(lat).format('%.1f'))
      });
    });
  }).flatten()
);
...
// var testRegion = ee.Geometry.Rectangle([150, -25, 160, -15]);
...
var viirsRaw = ee.Image(ee.Algorithms.If(
  viirsCol.size().gt(0),
  ee.Algorithms.If(
...
var viirsBand = ee.String(ee.Algorithms.If(viirsBandNames.contains('average'), 'average', viirsBandNames.get(0)));
...
if (AREA_METHOD === 'fractional') {
  ...
  var landArea1km = pixelArea
    ...
    .reduceResolution({ reducer: ee.Reducer.sum(), maxPixels: FRACTIONAL_MAX_PIXELS, bestEffort: true })
...
var areaResults = grid.map(function(cell) {
  ...
  var sumStats = sumStack.reduceRegion({ reducer: ee.Reducer.sum(), geometry: geom, scale: AREA_SCALE, maxPixels: 1e8, tileScale: TILE_SCALE });
  ...
  var landFraction = ee.Algorithms.If(cellAreaM2.gt(0), landAreaM2.divide(cellAreaM2).max(0).min(1), 0);
  var forestPct = ee.Algorithms.If(landAreaM2.gt(0), forestAreaM2.divide(landAreaM2).multiply(100), 0);
  return cell.set({
...
var landCells = areaResults.filter(ee.Filter.and(
...
var results = landCells.map(function(cell) {
  ...
  var lcHist = worldcover.reduceRegion({
    ...
    geometry: geom, scale: 1000, maxPixels: 1e8, tileScale: TILE_SCALE
  });
  var histRaw = lcHist.get('Map');
  var hist = ee.Dictionary(ee.Algorithms.If(histRaw, histRaw, ee.Dictionary({})));
  var histVals = hist.values();
  var totalPixels = ee.Number(ee.Algorithms.If(
    histVals.size().gt(0),
    histVals.reduce(ee.Reducer.sum()),
    ee.Number(0)
  ));
  ...
  function lcPct(classCode) {
    ...
    var count = ee.Number(ee.Algorithms.If(hist.contains(key), hist.getNumber(key), ee.Number(0)));
    return ee.Number(ee.Algorithms.If(totalPixels.gt(0), count.divide(totalPixels).multiply(100), ee.Number(0)));
  }
  ...
  var bestClass = landClassOrder.iterate(function(cls, best) {
    cls = ee.Number(cls);
    best = ee.List(best);
  ...
  var popTotal = ee.Algorithms.If(ee.Algorithms.IsEqual(popRaw, null), ee.Number(0), ee.Number(popRaw));
  ...
  var nightlightMean = ee.Algorithms.If(ee.Algorithms.IsEqual(meanRaw, null), ee.Number(-1), ee.Number(meanRaw));
  var p90Raw = nlResult.get('nightlight_p90');
  var p90 = ee.Algorithms.If(ee.Algorithms.IsEqual(p90Raw, null), ee.Number(-1), ee.Number(p90Raw));

  return cell.set({
    'landcover_class': ee.Algorithms.If(dominantClass, ee.Number(dominantClass).round(), null),
    'lc_pct_10': lcPct(10),   'lc_pct_20': lcPct(20),
    'lc_pct_30': lcPct(30),   'lc_pct_40': lcPct(40),
    'lc_pct_50': lcPct(50),   'lc_pct_60': lcPct(60),
    'lc_pct_70': lcPct(70),   'lc_pct_80': lcPct(80),
    'lc_pct_90': lcPct(90),   'lc_pct_95': lcPct(95),
...
// print('Oceania grids:', results.size());  // Skip .size() to avoid memory limit on large regions
Map.centerObject(region, 3);
Export.table.toDrive({
  collection: results, description: 'oceania_grid_q1', folder: 'geo_sonification',  // change q1→q2/q3/q4 per run
  fileNamePrefix: 'oceania_grid_q1', fileFormat: 'CSV',  // change q1→q2/q3/q4 per run (must match description)
  selectors: [
    'grid_id', 'lon', 'lat',
    'landcover_class',
    ...
    'population_total', 'land_area_km2',
    'nightlight_mean', 'nightlight_p90',
    'cell_area_km2', 'land_fraction'
  ]
});
