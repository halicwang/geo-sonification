...
const { parse: parseCSV } = require(path.join(__dirname, '..', 'server', 'node_modules', 'csv-parse', 'dist', 'cjs', 'sync.cjs'));
...
function assertGridResolution(csvContent, sourceLabel) {
  ...
  if (!nearlyEqual(lonStep, latStep)) {
    throw new Error(`[${sourceLabel}] Inconsistent resolution: lon step=${lonStep}, lat step=${latStep}.`);
  }
...
function getCsvHeader(csvContent) {
  ...
  const raw = (Array.isArray(headerRecords) && headerRecords.length > 0)
...
function hasLegacyLossColumns(headerSet) {
  return headerSet.has('loss_rate') ||
...
async function main() {
  ...
  for (const filename of GRID_FILES) {
    ...
    if (emptyCount > 0) {
      const emptyIndices = headerRaw.map((h, i) => (h === '' ? i + 1 : null)).filter((i) => i != null);
      console.warn(`[WARN] ${filename}: ${emptyCount} empty column name(s) at position(s) ${emptyIndices.join(', ')}`);
    }
    if (header.length !== new Set(header).size) {
      const seen = new Set();
      const dups = header.filter((h) => (seen.has(h) ? true : (seen.add(h), false)));
      console.warn(`[WARN] ${filename}: duplicate column name(s): ${[...new Set(dups)].join(', ')}`);
    }
    ...
    if (missing.length === 0) {
      ...
      if (lcPctPresent.length === LC_PCT_COLUMNS.length) {
        console.log(`  [INFO] V2 continuous landcover columns detected (${LC_PCT_COLUMNS.length} lc_pct_* columns)`);
      } else if (lcPctPresent.length === 0) {
        console.warn(`  [WARN] No lc_pct_* columns — will use discrete landcover_class fallback`);
      } else {
        ok = false;
        console.error(`  [FAIL] Incomplete lc_pct_* columns (found ${lcPctPresent.length}/${LC_PCT_COLUMNS.length})`);
        console.error(`    missing: ${LC_PCT_COLUMNS.filter((c) => !headerSet.has(c)).join(', ')}`);
      }

      try {
        const res = assertGridResolution(csv, filename);
        console.log(`  [INFO] Resolution OK: ${res.inferredGridSize}° (lon step=${res.lonStep}, lat step=${res.latStep})`);
      } catch (err) {
        ok = false;
      ...
      try {
        const sampleRecords = parseCSV(csv, {
          columns: true, skip_empty_lines: true, relax_quotes: true, bom: true, trim: true, to_line: 11
        });
        let gridIdIssues = 0;
        ...
        if (gridIdIssues > 0) {
          console.warn(`  [WARN] ${gridIdIssues} rows have mismatched or malformed grid_id (checked first ${sampleRecords.length} rows)`);
        }
      } catch (gridIdErr) {
    ...
    ok = false;
    const legacyHint = hasLegacyLossColumns(headerSet) ? ' (legacy loss_* / forest2000_* detected)' : '';
