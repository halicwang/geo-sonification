...
function parsePort(envVar, defaultPort, name) {
    ...
    if (isNaN(port) || port < 1 || port > 65535) {
        console.error(`ERROR: Invalid ${name} port "${value}". Must be a number between 1 and 65535.`);
        process.exit(1);
    }
    return port;
}
...
const USE_LEGACY_AGGREGATION = process.env.USE_LEGACY_AGGREGATION === '1' || process.env.USE_LEGACY_AGGREGATION === 'true';
...
const LAND_FRACTION_WEIGHT_MODE = String(process.env.LAND_FRACTION_WEIGHT_MODE || 'identity').toLowerCase();
const LAND_FRACTION_WEIGHT_EXP = parseNonNegativeFloat('LAND_FRACTION_WEIGHT_EXP', 1, 'LAND_FRACTION_WEIGHT_EXP');
...
console.log(`[Aggregation] version=${AGGREGATION_VERSION} config=${JSON.stringify(AGGREGATION_CONFIG)}`);
...
const PER_GRID_THRESHOLD_CENTER = parseNonNegativeInt('PER_GRID_THRESHOLD', 50, 'PER_GRID_THRESHOLD');
...
const BROADCAST_STATS = process.env.BROADCAST_STATS === '1' || process.env.BROADCAST_STATS === 'true';
