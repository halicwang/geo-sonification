...
const { DISABLE_CACHE, FORCE_REBUILD_CACHE, AGGREGATION_VERSION, AGGREGATION_CONFIG, GRID_SIZE } = require('./config');
...
function assertCsvSchema(csvHeader, sourceLabel) {
    ...
    const hasLegacyLoss = headerSet.has('loss_rate') || headerSet.has('loss_km2') || headerSet.has('loss_density') || headerSet.has('forest2000_km2');
...
function deduplicateGrids(grids) {
    ...
    for (const grid of grids) {
        ...
        if (!seen.has(key)) {
            ...
            const gridHasLC = hasContinuousLcData(grid) || (grid.landcover_class != null && grid.landcover_class >= 10);
            const existingHasLC = hasContinuousLcData(existing) || (existing.landcover_class != null && existing.landcover_class >= 10);
            ...
            if (gridHasLC && !existingHasLC) {
                ...
                const existingLand = Number.isFinite(existing.land_area_km2) ? existing.land_area_km2 : 0;
...
function assertGridResolution(gridData, sourceLabel) {
    ...
    if (lonStep == null || latStep == null) {
        console.warn(`[Grid] Could not infer data resolution from ${sourceLabel}; skipping GRID_SIZE consistency check.`);
        return;
    }

    console.log(`[Grid] Inferred resolution from ${sourceLabel}: lon step=${lonStep}, lat step=${latStep}`);
    ...
    return grids.map(g => {
        const out = {
            ...
            land_fraction: Number.isFinite(g.land_fraction)
                ...
                : (g.cell_area_km2 > 0 ? Math.max(0, Math.min(1, g.land_area_km2 / g.cell_area_km2)) : 0),
            nightlight_mean: Number.isFinite(g.nightlight_mean) ? g.nightlight_mean : NIGHTLIGHT_NO_DATA,
            nightlight_p90: Number.isFinite(g.nightlight_p90) ? g.nightlight_p90 : NIGHTLIGHT_NO_DATA,
            population_density: Number.isFinite(g.population_density)
                ? g.population_density
                : (Number.isFinite(g.land_area_km2) && g.land_area_km2 > 0 && Number.isFinite(g.population_total)
...
async function loadGridData() {
    ...
    const assertGridSchema = (sample, sourceLabel) => {
        if (!sample || typeof sample !== 'object') {
            const err = new Error(`[Cache] Unable to read cache data: ${sourceLabel}\nPlease delete data/cache/ and retry.`);
    ...
    if (!DISABLE_CACHE && !FORCE_REBUILD_CACHE) {
        try {
            ...
            if (Array.isArray(cached)) {
                ...
                if (cached.schemaVersion !== CACHE_SCHEMA_VERSION) {
                    console.log(`[Cache] Schema version mismatch (cache=${cached.schemaVersion}, expected=${CACHE_SCHEMA_VERSION}), invalidating...`);
        ...
        console.log('[CSV] Cache disabled, loading from CSV files (cache disabled, skipping write)');
    }
    ...
    if (continuousCount > 0 && discreteCount > 0) {
        console.log(`[Data] Landcover mode: MIXED (${continuousCount} continuous, ${discreteCount} discrete) — per-cell fallback active`);
