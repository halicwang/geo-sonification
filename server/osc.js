 ...
 *
 * When ENABLE_OSC=false, this module exports no-op stubs and never opens
 * a UDP port. The null-object interface is identical to the real one.
 *
 * UDP is fire-and-forget; there is no delivery guarantee.
 */

const { ENABLE_OSC, OSC_HOST, OSC_PORT, DEBUG_OSC, GRID_SIZE } = require('./config');

// ── Null-object path: no UDP port, all sends are no-ops ──
if (!ENABLE_OSC) {
    module.exports = {
        isOscReady: () => false,
        sendToMax: () => {},
